...
export default class EditConcert extends Component{
    ...
    componentDidMount(){
        ...
        .then(response =>{
            this.setState({
                zenekar: response.data[0].zenekar,
                ido: response.data[0].ido,
                szinpad: response.data[0].szinpad,
                tomeg: response.data[0].tomeg,
            })
        })
    ...
    onSubmit(e){
        ...
        console.log(koncert);
        axios.post('http://localhost:5000/update/'+this.props.match.params.id, koncert)
        .then(res=>{
            console.log(res.data)
            if (res.data === "Módosítás sikerült") {
                window.location = '/';
            }
        });
    }
        ...
        return(
            <div>
                ...
                <form onSubmit={this.onSubmit}>
                    ...
                    <div className="form-group">
                    <input type="submit" value="Koncert módosítása" className="btn btn-primary" />
                    </div>
                </form>
            </div>
        )
